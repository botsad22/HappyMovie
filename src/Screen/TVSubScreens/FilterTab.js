...
export default class FilterTab extends Component {
  ...
  fetchAPI = () => {
    ...
    for(i=0; i<this.state.selectedGenresItems.length; i++){
      params += i != this.state.selectedGenresItems.length? this.state.selectedGenresItems[i] + "," : this.state.selectedGenresItems[i];
    }
    
    if(this.state.selectedYearItems.length >0)
      params += params == "" ? "y=" : "&y=";
    for(i=0; i<this.state.selectedYearItems.length; i++){
      params += i != this.state.selectedYearItems.length - 1? this.state.selectedYearItems[i] + "," : this.state.selectedYearItems[i];
    }
    if(this.state.selectedRatingItems.length > 0)
      params += (params == "" ? "r=" : "&r=") + this.state.selectedRatingItems[0];
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.buttons}>
          <TouchableOpacity onPress={this.back.bind(this)} style={{width:screenWidth/2}} disabled={this.state.firstLoaded}>
            <Button
              title="BACK"
              color="#9f9f9f"
          ...
          <TouchableOpacity onPress={this.fetchAPI} style={{width:screenWidth/2}}>
            <Button
              title="SUBMIT"
